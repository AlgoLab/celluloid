...
def cluster_and_output(k, matrix, clust_type, inputpath, outdir):
    model = Birch(branching_factor=20, n_clusters=k, threshold=0.5,
            compute_labels=True).fit(matrix)
    # print(matrix.shape)
    ...
    with open('{0}_birch.mutations'.format(outfile), 'w+') as file_out:
            for cluster in sorted(labels_to_index):
                file_out.write('{}\n'.format(
                    ','.join([ str(x+1) for x in labels_to_index[cluster]])
                ))

    with open('{0}_birch_clusters.txt'.format(outfile), 'w+') as file_out:
        for cluster in sorted(labels_to_index):
            file_out.write('{0}\t"{1}"\n'.format(
    ...
    np.savetxt('{0}_birch.matrix'.format(outfile), out_matrix, fmt='%d', delimiter=' ')
