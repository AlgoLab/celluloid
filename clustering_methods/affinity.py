...
def cluster_and_output(k, matrix, clust_type, inputpath, outdir):
    model = AffinityPropagation().fit(matrix)
    # print(matrix.shape)
    ...
    #         stats[model.cluster_centers_[i,j]] += 1

    
    filename = os.path.splitext(os.path.basename(inputpath))[0]
    outfile = os.path.join(outdir, filename)

    with open('{0}_affinity.mutations'.format(outfile), 'w+') as file_out:
            for cluster in sorted(labels_to_index):
                file_out.write('{}\n'.format(
                    ','.join([ str(x+1) for x in labels_to_index[cluster]])
                ))

    with open('{0}_affinity_clusters.txt'.format(outfile), 'w+') as file_out:
        for cluster in sorted(labels_to_index):
            file_out.write('{0}\t"{1}"\n'.format(
    ...
    np.savetxt('{0}_affinity.matrix'.format(outfile), out_matrix, fmt='%d', delimiter=' ')
